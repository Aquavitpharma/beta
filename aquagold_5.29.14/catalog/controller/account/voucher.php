...
class ControllerAccountVoucher extends Controller { 
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$giftcard_by = ($this->request->post['doctor_name']) ? $this->request->post['doctor_name'] : $this->request->post['to_name'];
			$this->session->data['vouchers'][mt_rand()] = array(
				'description'      => sprintf($this->language->get('text_for'), $this->currency->format($this->currency->convert($this->request->post['amount'], $this->currency->getCode(), $this->config->get('config_currency'))), $giftcard_by),
				...
				'doctor_name'      => $this->request->post['doctor_name'],
				'amount'           => $this->currency->convert($amount, $this->currency->getCode(), $this->config->get('config_currency'))
			);
		...
		$this->data['discount'] = $this->url->link('account/coupon', '', 'SSL');
		$this->data['product'] = $this->url->link('product/product', 'product_id=50');
		$this->data['search'] = $this->url->link('information/tellafriend', '', 'SSL');
		$this->data['terms'] = $this->url->link('information/information/info', 'information_id=13');
		...
		$this->data['discount_rate'] = $this->model_account_coupon->getDiscount();
		...
		if (isset($this->request->post['doctor_name'])) {
			$this->data['doctor_name'] = $this->request->post['doctor_name'];
		} else {
