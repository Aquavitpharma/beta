...
class ModelCheckoutVoucher extends Model {
	public function addVoucher($order_id, $data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "voucher SET order_id = '" . (int)$order_id . "', code = '" . $this->db->escape($data['code']) . "', from_name = '" . $this->db->escape($data['from_name']) . "', from_email = '" . $this->db->escape($data['from_email']) . "', to_name = '" . $this->db->escape($data['to_name']) . "', to_email = '" . $this->db->escape($data['to_email']) . "', voucher_theme_id = '" . (int)$data['voucher_theme_id'] . "', message = '" . $this->db->escape($data['message']) . "', shipping_address = '" . $this->db->escape($data['shipping_address']) . "', doctor_name = '" . $this->db->escape($data['doctor_name']) . "', amount = '" . (float)$data['amount'] . "', status = '1', date_added = NOW()");
	...
	public function confirm($order_id) {
		...
		if ($order_info) {
			...
			foreach ($voucher_query->rows as $voucher) {
				...
				//$mail->setTo($voucher['to_email'].','.$voucher['from_email']);
	...
	public function transaction($order_id, $description, $amount) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "customer_transaction` SET customer_id = '" . (int)$this->customer->getId() . "', order_id = '" . (int)$order_id . "', description = '" . $description . "', amount = '" . (float)$amount . "', date_added = NOW()");
	}
	
	public function reward($code, $order_id, $description, $points) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "customer_reward` SET code = '" . $code . "', customer_id = '" . (int)$this->customer->getId() . "', order_id = '" . (int)$order_id . "', description = '" . $description . "', points = '" . (int)$points . "', date_added = NOW()");
	}
	
	public function getTotalRewards($code) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "customer_reward` WHERE customer_id = '" . (int)$this->customer->getId() . "' AND code = '" . $code . "'");
