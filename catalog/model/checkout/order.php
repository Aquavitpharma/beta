...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		foreach ($data['vouchers'] as $voucher) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "order_voucher SET order_id = '" . (int)$order_id . "', description = '" . $this->db->escape($voucher['description']) . "', code = '" . $this->db->escape($voucher['code']) . "', from_name = '" . $this->db->escape($voucher['from_name']) . "', from_email = '" . $this->db->escape($voucher['from_email']) . "', to_name = '" . $this->db->escape($voucher['to_name']) . "', to_email = '" . $this->db->escape($voucher['to_email']) . "', voucher_theme_id = '" . (int)$voucher['voucher_theme_id'] . "', message = '" . $this->db->escape($voucher['message']) . "', shipping_address = '" . $this->db->escape($voucher['shipping_address']) . "', doctor_name = '" . $this->db->escape($voucher['doctor_name']) . "', amount = '" . (float)$voucher['amount'] . "'");
		}
	...
	public function confirm($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		if ($order_info && !$order_info['order_status_id']) {
			...
			if ($this->config->get('config_complete_status_id') == $order_status_id) {
				...
				$this->model_checkout_voucher->reward($order_id, 'Aquagold fine touch', $order_info['total']/10);
			}
			...
			if($order_product_query->num_rows > 0) {
				$subject = sprintf($language->get('text_new_subject'), 'AQUAGOLD fine touch');
			} else {
				$subject = sprintf($language->get('text_new_subject'), 'Gift card');
			}
